...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes (size) {
    ...
    // Save the collection of DOM nodes to the variable randomPizzas, and using it in the for loop without querying the DOM each time.
    var randomPizzas = document.querySelectorAll(".randomPizzaContainer");
      
    for (var i = 0; i < randomPizzas.length; i++) {
      // Set the width of each element to that percentage.
...
function updatePositions() {
  ...
  // An aray to maintain all possible horizontal pizza positions.
  var pizzaArray = new Array();

  var itemsLength = items.length;
  var phaseHolder = document.body.scrollTop / 1250;

  // A separate for loop to push all phase values into pizzaArray.
  // Also, kept pizza style recalculation in a different loop to avoid unnecessary recalculation.
...
// Global items declaration so it can be accessed by updatePositions.
var items;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Plus, moved items outside the function so as to query items after they've been created but before they're needed.
